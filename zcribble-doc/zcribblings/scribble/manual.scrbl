#lang zcribble/doc
@(require zcribble/manual "utils.rkt"
          (for-syntax racket/base)
          (for-label zcribble/manual-struct
                     racket/list
                     version/utils
                     syntax/quote))
...
@defmodulelang[zcribble/manual]{The @racketmodname[zcribble/manual]
language provides all of @racketmodname[zcribble/base] plus many
...
The @racketmodname[zcribble/manual] name can also be used as a library
...
@; ------------------------------------------------------------------------
@section[#:tag "zcribble:manual:code"]{Typesetting Code}
...
See also @racketmodname[zcribble/comment-reader].
...
@defmodulereader[zcribble/comment-reader]

As a reader module, @racketmodname[zcribble/comment-reader] reads a
...
@racketmodname[zcribble/comment-reader] extends the current reader to
...
@racketmodname[zcribble/comment-reader] a chance to convert them.

The implementation of @racketmodname[zcribble/comment-reader] uses @racket[unsyntax] to
typeset comments. When using @racketmodname[zcribble/comment-reader]
...
@racketmodname[zcribble/comment-reader] with @racket[#:escape-id]. For example,

@verbatim[#:indent 2]|{
  @#reader zcribble/comment-reader #:escape-id UNSYNTAX
   (RACKETBLOCK
     (define-syntax (m stx)
       (syntax-case stx ()
         [(_ x)
...
in @racket[secref].
For example:
@racketblock[@tech[#:doc '(lib "zcribblings/reference/reference.scrbl")]{blame object}]

creates a link to @tech[#:doc '(lib "zcribblings/reference/reference.scrbl")]{blame object} in
@other-doc['(lib "zcribblings/reference/reference.scrbl")].
...
See also @secref["base-indexing"] for @racketmodname[zcribble/base].
...
@margin-note{See also @racketmodname[zcriblib/autobib].}
...
style configuration of @racket[@#,hash-lang[] @#,racketmodname[zcribble/manual]].
...
@defmodule[zcribble/manual-struct]{The
@racketmodname[zcribble/manual-struct] library provides types used to
describe index entries created by @racketmodname[zcribble/manual]
functions. These structure types are provided separate from
@racketmodname[zcribble/manual] so that
@racketmodname[zcribble/manual] need not be loaded when deserializing
...
Using @racket[@#,hash-lang[] @#,racketmodname[zcribble/manual]] for the
