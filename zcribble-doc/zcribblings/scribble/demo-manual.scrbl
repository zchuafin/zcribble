...
@racketmodname[zcribble/base] style class for the
...
style class is from @racketmodname[zcribble/base] and the
@racket['boxed] style name on a table. The @css{RBoxed} style class is
from the @racket[zcribble/manual] layer. Both @css{boxed} and @css{RBoxed}
are used for all definition boxes by @racket[zcribble/manual] forms.

The initial content of the table includes a @css{SubFlow} (a
@racket[zcribble/base] style class for non-indented flow) to combine
