#lang zcribble/doc
@(require zcribble/manual zcribble/core zcribble/decode
          zcribble/html-properties zcribble/latex-properties
          "utils.rkt"
...
@itemize[
 ...
 @item{You may need to produce a document whose page layout is
       ...
       kind of configuration, you can run the @exec{zcribble} command-line
...
exports of @racket[zcribble/base]. You can use them or redefine
them. The styles are specified by @filepath{zcribble.css} and
@filepath{zcribble.tex} in the @filepath{zcribble} collection.

The styles used by @racketmodname[zcribble/manual] are implemented by
@filepath{racket.css} and @filepath{racket.tex} in the
@filepath{zcribble} collection. Other libraries, such as
@racketmodname[zcriblib/autobib], similarly implement styles through files
...
Latex packages, and future styles needed by @racketmodname[zcribble/base] and
@racketmodname[zcribble/manual] will start with @litchar{S}.
...
@itemize[
 ...
 @item{A @deftech{prefix file} determines the @tt{DOCTYPE} line for
       ...
       The default prefix files are @filepath{zcribble-prefix.html}
       and @filepath{zcribble-prefix.tex} in the @filepath{zcribble}
       collection.}

 @item{A @deftech{style file} refines the implementation of styles
       used in the document---typically just the ``built-in'' styles
       used by @racketmodname[zcribble/base].

       The default style files, @filepath{zcribble-style.css} and
       @filepath{zcribble-style.tex} in the @filepath{zcribble}
...
When rendering a document through the @exec{zcribble} command-line
...
overridden using @exec{zcribble} command-line flags. Furthermore,
languages like @racketmodname[zcribble/manual] and
@racketmodname[zcribble/sigplan] add a @racket[html-defaults] and/or
...
of the CSS style classes used by @racketmodname[zcribble/base] forms and
functions:

@itemlist[

 @item{@other-doc['(lib "zcribblings/zcribble/demo-s1.scrbl")] shows
       the default style in a single-page rendering without a search
       box.}

 @item{@other-doc['(lib "zcribblings/zcribble/demo-m1.scrbl")] shows
       the default style in a multi-page rendering without a search
       box.}

 @item{@other-doc['(lib "zcribblings/zcribble/demo-s2.scrbl")] shows
       the current manual style's adjustments in a single-page
       rendering with a search box.}

 @item{@other-doc['(lib "zcribblings/zcribble/demo-m2.scrbl")] shows
...
zcribble HTML output to hide an element on Internet Explorer 6.
...
@racketmodname[zcribble/manual] forms and functions in addition to the
@seclink["builtin-css"]{base style classes}.

@itemlist[

 @item{@other-doc['(lib "zcribblings/zcribble/demo-manual-s1.scrbl")] shows
       the original style in a single-page rendering without a search
       box.}

 @item{@other-doc['(lib "zcribblings/zcribble/demo-manual-m1.scrbl")] shows
       the original style in a multi-page rendering without a search
       box.}

 @item{@other-doc['(lib "zcribblings/zcribble/demo-manual-s2.scrbl")] shows
       the current manual style's adjustments in a single-page
       rendering with a search box.}

 @item{@other-doc['(lib "zcribblings/zcribble/demo-manual-m2.scrbl")] shows
       the current manual style's adjustments in a multi-page
       rendering with a search box.}
...
The @filepath{zcribble.tex} Latex configuration includes several
...
@itemlist[
 ...
 @item{@ltxd[0]{preDoc} --- called before the document content; the 
       default does nothing, while the @racketmodname[zcribble/manual]
 ...
 @item{A set of commands that control the basic set of Latex packages that are loaded:
  ...
  @history[#:changed "1.37" @list{
              ...
              See @hyperlink["https://github.com/racket/zcribble/pull/274"]{
              racket/zcribble#274} for examples.}]}

 @item{@ltxd[0]{sectionNewpage} --- called before each top-level
       section starts; the default does nothing, while the
       @racketmodname[zcribble/manual] configuration uses
 ...
 @item{@ltxd[2]{SecRef} --- like @ltx{SecRefLocal}, but used when the
       ...
       @racketmodname[zcribble/manual] redefinition of this macro
...
@defmodule[zcribble/latex-prefix]{Provides a string that is useful for
