...
@(require (except-in "utils.rkt" author) (for-label zcribble/jfp))

@(define-syntax-rule (def base-author)
   (begin
     (require (for-label zcribble/base))
     (define base-author @racket[author])))
@(def base-author)

@title{JFP Paper Format}

@defmodulelang[zcribble/jfp]{The @racketmodname[zcribble/jfp]
...
@racket[zcribble/jfp] collections, then it is downloaded on demand to
@racket[(find-system-path 'addon-dir)].}

Latex output with @racketmodname[zcribble/jfp] uses a main-document
...
A replacement for @base-author from @racketmodname[zcribble/base].}
