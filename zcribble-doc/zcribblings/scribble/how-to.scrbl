...
@(define pkg-doc '(lib "pkg/zcribblings/pkg.scrbl"))

@title[#:tag "how-to-doc"]{Getting Started with Documentation}

Although the @exec{zcribble} command-line utility generates output
...
@margin-note{See @secref["getting-started"] for information on using the
             @exec{zcribble} command-line utility.}
...
@codeblock|{
  ...
  @(require (for-label racket))
  @(define ref-src
     '(lib "zcribblings/reference/reference.scrbl"))
...
The @racket[examples] form from @racket[zcribble/eval] helps you
