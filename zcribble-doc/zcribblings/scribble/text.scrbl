...
@(require zcribble/manual
          zcribble/core zcribble/html-properties zcribble/latex-properties
          (except-in "utils.rkt" splice begin include)
          (for-label (except-in racket/base begin)
                     zcribble/text
                     (only-in zcribble/text
                              [begin/text begin]
                              [include/text include])))
@initialize-tests

@(define (tech/r s) @tech[s #:doc '(lib "zcribblings/reference/reference.scrbl")])
@(define scribble-doc '(lib "zcribblings/zcribble/zcribble.scrbl"))
...
@defmodulelang[zcribble/text]{The @racketmodname[zcribble/text]
...
When @racketmodname[zcribble/text] is used via @racket[require]
...
@racket[zcribble/text] language be read as strings, which get printed
...
Using the zcribble @seclink[#:doc scribble-doc "reader"]|{@-forms}| syntax, you can write
...
In fact, this is common enough that the @racket[zcribble/text]
...
@racket[zcribble/text] language, is any value-producing expression.)
...
output.  The @racket[zcribble/text] language provides a few functions
...
@racket[(require zcribble/text)], which will get all of the bindings
that are available in the @racket[zcribble/text] language.  Using
...
@example|-{#lang at-exp racket/base
           (require zcribble/text racket/list)
           (define (itemize . items)
             (add-between (map (lambda (item)
                                 @list{* @item})
                               items)
...
mostly-text file from a @racket[zcribble/text] source file.  It might be
...
external editor).  In these cases, the @racket[zcribble/text] language
...
(Using @racket[require] with a text file in the @racket[zcribble/text]
...
@;@racket[zcribble/text].  This is similar to using @racket[load] in a
...
@racketmodname[zcribble/text]. If @racket[command-char] is supplied,
then it replaces @litchar["@"] as the escape character.

The @racketmodname[zcribble/text] language via @hash-lang[] provides
