...
@(require zcribble/manual 
          ...
          (only-in zcribble/eval as-examples)
          (for-label zcribble/manual-struct
                     ...
                     net/url-structs
                     zcriblib/figure
                     (only-in zcribble/html-render render-mixin)))

@title[#:tag "core"]{Structures And Processing}

@defmodule[zcribble/core]

A document is represented as a @techlink{part}, as dezcribed in
...
The @racketmodname[zcribble/tag] library provides functions for constructing
@tech{tags}.
...
@itemize[
 ...
 @item{@racket[document-date] structure --- A date for the part,
  ...
  @item{@racket[body-id] structure --- Generated HTML uses the given
        ...
        default is @racket["zcribble-racket-lang.org"], but
 ...
 @item{@racket['enable-index-merge] --- On an index parts or one of
       ...
       references. See also @racketmodname[zcriblib/book-index].}
...
@margin-note*{See also @racket[cond-block] in @racketmodname[zcriblib/render-cond].}
@;
The symbol @indexed-racket['zcribble:current-render-mode] is
...
@racket['zcribble:current-render-mode] cannot be changed via
@racket[_set].}
...
@itemlist[
 ...
 @item{For Latex/PDF output, the generated reference's format can
       ...
       uppercase if the element's style includes a @racket['uppercase]
       property. The @racketmodname[zcribble/manual] style uses the
...
@racketmodname[zcribble/manual-struct] for other typical types of
@racket[desc] values.
...
@margin-note*{See also @racket[cond-element] in @racketmodname[zcriblib/render-cond].}
@;
...
from @racketmodname[zcriblib/figure].
...
@defmodule[zcribble/html-properties]{ The
@racket[zcribble/html-properties] library provides datatypes used as
...
@racketmodname[zcribble/manual] style recognizes those tags to make
...
@exec{zcribble} command-line tool's @DFlag{html} and
@DFlag{htmls} modes.}
...
@defmodule[zcribble/latex-properties]{ The
@racket[zcribble/latex-properties] library provides datatypes used as
...
@exec{zcribble} command-line tool for @DFlag{latex} or @DFlag{pdf}
...
be a result of @racket[path->main-collects-relative].
 ...
 @racketmodname[zcribble/manual] and
 @racketmodname[zcribble/sigplan] add this property to a
 document to specify appropriate files for Latex rendering.
 With @racketmodname[zcribble/base], not specifying a
 @racket[latex-defaults] struct is equivalent to using this
 one:
 @racketblock[
 (latex-defaults
  (list 'collects #"zcribble" #"zcribble-prefix.tex")
  (list 'collects #"zcribble" #"zcribble-style.tex")
  '())]

See also @racketmodname[zcribble/latex-prefix].}
...
@defstruct[(latex-defaults+replacements latex-defaults)
 ...
 @racket["zcribble-load-replace.tex"] to
 @racket["my-zcribble.tex"], then the
 @racket["my-zcribble.tex"] file in the current directory
 will we used in place of the standard zcribble package
 inclusion header. Using @racket["zcribble-load-replace.tex"]
 can disable the use of possibly-conflicting packages in the
 LaTeX output. The file
 @racket[(collection-file-path "zcribble" "zcribble.tex")]
 contains a number of macros of the form @tt{\packageMathabx}
 that will, by default, load the corresponding packages. Use
 a @racket["zcribble-load-replace.tex"] replacement
 with content like
