...
@(begin
  (define-syntax-rule (def-html-render-mixin id mid)
    (begin
      (require (for-label zcribble/html-render))
...
@(begin
  (define-syntax-rule (def-latex-render-mixin id)
    (begin
      (require (for-label zcribble/latex-render))
      (define id @racket[render-mixin])))
...
default, it is @html:render-mixin from @racketmodname[zcribble/html-render].
...
@racketmodname[zcribble/html-render], so they should be
...
@defmodule[zcribble/base-render]{The
@racketmodname[zcribble/base-render] module provides @racket[render%],
which implements the core of a renderer. This rendering class must be
refined with a mixin from @racketmodname[zcribble/text-render],
@racketmodname[zcribble/markdown-render], or
@racketmodname[zcribble/html-render], or
@racketmodname[zcribble/latex-render].}
...
as @racketmodname[zcribble/manual], generally do not call the render
...
@defmodule/local[zcribble/text-render]{
...
@defmodule/local[zcribble/markdown-render]{
...
@defmodule/local[zcribble/html-render]{
...
@defmixin[render-mixin (render<%>) ()]{
  ...
  @defconstructor/auto-super[([search-box? boolean? #f])]{
   ...
   is created with @racket[zcribble/manual], then it will be
...
@defmodule/local[zcribble/latex-render]{
...
@defmodule/local[zcribble/pdf-render]{

@defmixin[render-mixin (render<%>) ()]{

Specializes a @racket[render<%>] class for generating PDF output via
Latex, building on @|latex:render-mixin| from @racketmodname[zcribble/latex-render].}
...
@defmodule/local[zcribble/contract-render]{
...
via a @racketmodname[zcribble/text-render] renderer).
