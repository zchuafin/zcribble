...
@(require zcribble/manual zcribble/bnf "utils.rkt"
          pict
          (for-label zcriblib/figure zcribble/base (only-in zcribble/acmart abstract)
...
@racketmodname[zcribble/base]] indicates that the file implements a
...
for the kind of document that you want as output:
       ...
       @itemize[
         ...
         @item{Run
                @commandline{zcribble --pdf mouse.scrbl}
               ...
               If you'd like to see the intermediate Latex, try
                @commandline{zcribble --latex mouse.scrbl}
               to generate @filepath{mouse.tex}.}

          ]

See @secref["running"] for more information on the @exec{zcribble}
command-line tool.
...
Run the @exec{zcribble} command(s) from @secref["first-example"]
...
rendered using @exec{zcribble}. Running @exec{zcribble} on
...
replace the @filepath{zcribble.css} file for the generated pages, and
...
Using @racketmodname[zcribble/acmart] or
@racketmodname[zcribble/manual] does not change the rendered HTML for
a document---aside from @racketmodname[zcribble/manual] adding a
version number---but it changes the set of bindings available in the
document body. For example, with @racketmodname[zcribble/acmart], the
...
@racketmodname[zcribble/base] or @racketmodname[zcribble/manual]
...
@racketmodname[zcribble/acmart], then @racket[abstract] becomes
...
The @racketmodname[zcribble/base] language provides a collection of
basic operations (both @racketmodname[zcribble/acmart] and
@racketmodname[zcribble/manual] are supersets of
@racketmodname[zcribble/base]). Many of the operations are style
...
language like @racketmodname[zcribble/base] (as opposed to
...
@racketmodname[zcribble/base]], the top level is a text-mode stream,
...
@racketmodname[zcribble/base], @racketmodname[zcribble/manual], and
@racketmodname[zcribble/acmart] languages all use the same
@racket[decode] operation.  The @racketmodname[zcribble/text] language,
...
@racketmodname[zcribble/text], see @other-doc['(lib
"zcribblings/zcribble/zcribble-pp.scrbl")].)

@margin-note{More precisely, languages like
             @racketmodname[zcribble/base] apply @racket[decode] only after
...
@other-doc['(lib "zcribblings/zcribble/zcribble-pp.scrbl")].
