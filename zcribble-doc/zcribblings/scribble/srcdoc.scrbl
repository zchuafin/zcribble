...
@(require zcribble/manual "utils.rkt"
          (for-label zcribble/srcdoc zcribble/extract racket/contract))

@title[#:tag "srcdoc"]{In-Source Documentation}

The @racketmodname[zcribble/srcdoc] and
@racketmodname[zcribble/extract] libraries support writing
...
The @racketmodname[zcribble/srcdoc] library provides forms for
exporting a binding with associated documentation. The
@racket[zcribble/extract] library is used to pull
@racket[zcribble/srcdoc]-based documentation into a Scribble document
(perhaps for multiple libraries).

Although documentation is written with a library's implementation when
using @racketmodname[zcribble/srcdoc], the documentation creates no
...
@defmodule[zcribble/srcdoc]

Documentation information generated by @racketmodname[zcribble/srcdoc]
forms are accumulated into a @racketidfont{srcdoc} submodule. The
generated submodule is accessed by the bindings of
@racketmodname[zcribble/extract].
...
Typically, a library that uses @racketmodname[zcribble/srcdoc]
includes at least @racket[(require (for-doc zcribble/base zcribble/manual))]
...
@racketmodname[zcribble/srcdoc] (so that the module has a
