      ...
      [(_ id contract names desc)
       (with-syntax ([header                      
         ...
         (values
          #'[id contract]
          #'(defproc* header . desc)
          #'((only-in zcribble/manual defproc*))
          #'id))])))
      ...
      [(_ id (parameter/c contract) arg-id desc)
       (begin
         ...
         (values
          ...
          #'((only-in zcribble/manual defparam))
          #'id))])))
     ...
     (let ()
       ...
       (values
        ...
        #`(#,result-form struct-name ([field-name contract-expr-datum] ...)
                         #,@(reverse ds-args)
                         #,@desc)
        #`((only-in zcribble/manual #,result-form))
        the-name))]))
      ...
      [(_ id contract desc)
       (begin
         ...
         (values
          ...
          #'((only-in zcribble/manual defthing))
          #'id))])))
       ...
       (with-syntax ([id (if (syntax-e #'d.defined-id)
         ...
         (values
          ...
          #'(defform 
              ...
              c.contracts-seq ...
              . desc)
          #'((only-in zcribble/manual defform))
          #'id))])))
      ...
      [(_ id super (intf-id ...) pre-flow)
       (begin
         ...
         (values
          ...
          #'((only-in zcribble/manual defclass defconstructor defmethod this-obj))
          #'id))])))
