...
(define-serializable-struct title-decl (tag-prefix tags version style content) #:transparent)
(define-serializable-struct part-start (depth tag-prefix tags style title) #:transparent)
(define-serializable-struct splice (run) #:transparent)
(define-serializable-struct part-index-decl (plain-seq entry-seq) #:transparent)
(define-serializable-struct part-collect-decl (element) #:transparent)
(define-serializable-struct part-tag-decl (tag) #:transparent)

(provide
 (struct-out title-decl)
 (struct-out part-start)
 (struct-out splice)
 (struct-out part-index-decl)
 (struct-out part-collect-decl)
 (struct-out part-tag-decl))
